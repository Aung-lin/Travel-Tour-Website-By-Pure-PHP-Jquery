...
if(isset($_POST['delete']))
{
    ...
    if($query_run)
    {
        ...
        echo "<script>window.location='AdminReview.php'</script>";
    }
    else
    {
...
if(isset($_POST["ApproveReview"])){
    ...
    $id = $_POST['ApproveReview'];
    

  
  $TrunStatusTrue = "UPDATE reviews set status = true where ReviewId= $id";
  $Runquery_SelectStatus= mysqli_query($connect, $TrunStatusTrue);
  

  if($Runquery_SelectStatus)
  {
  ...
  <body>
     ...
     <div class="span_1_of_5 sidenav">
      ...
      <div class="nav-items">
        <ul class="navbar-nav">
          <li class="nav-item">
            <a id="sixthChild" href="dashboard2.php">
          ...
          <li class="nav-item">
            <a id="firstChild" href="AdminReview.php">
              <span class="nav-link"
...
<div class="clearfix"></div>
    ...
    <h2>Data Table | For Reviews View</h2>
...
if($Run_SelectQuery){
  while($row= mysqli_fetch_array($Run_SelectQuery)){
    ...
    <td> <?php echo $row['ReviewParagraph']; ?></td>
    <td> <?php echo $row['ReviewDate']; ?></td>
    <td><?php echo $row['Status']?'Shared':'Still NOt Shared'; ?></td>


    <form class="" action="AdminReview.php" method="post" autocomplete="off" enctype="multipart/form-data">
    <input type="hidden" name="ApproveReview" value="<?php echo $row['ReviewId'];?>">
    <td><button type="submit" name="BtnApprove" id="update"><i class="fa-solid fa-share"></i></button></td>
    </form>
   
    <form class="" action="AdminReview.php" method="post" autocomplete="off" enctype="multipart/form-data">
    <input type="hidden" name="delid" value="<?php echo $row['ReviewId'];?>">
